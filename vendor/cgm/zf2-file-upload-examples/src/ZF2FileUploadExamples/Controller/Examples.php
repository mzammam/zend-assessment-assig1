...
class Examples extends AbstractActionController
{
    ...
    protected function redirectToSuccessPage($formData = null)
    {
        ...
        $response = $this->redirect()->toRoute('fileupload/posttotwitter');
        $response->setStatusCode(303);
        return $response;
       /* $this->sessionContainer->formData = $formData;
        $response = $this->redirect()->toRoute('posttotwitter', array('user_id'=>5));
        $response->setStatusCode(303);
        return $response;*/
    }
    ...
    public function postimageAction()
    {
        ...
        "message"=>$this->getEvent()->getRouteMatch()->getParam('tweet'),
        "picture"=>"http://localhost".$this->getRequest()->getbaseUrl()."/public/uploads/".$this->getEvent()->getRouteMatch()->getParam('path'),
        ) );
       var_dump("http://localhost".$this->getRequest()->getbaseUrl()."/public/uploads/".$this->getEvent()->getRouteMatch()->getParam('path'));
       $path  =  $this->getEvent()->getRouteMatch()->getParam('path');
